...
def read_prc_csv(tic, start, end, prc_col='Adj Close'):
    ...
    tic = tic.lower()
    file_path = os.path.join(DATADIR, f'{tic}_prc.csv')
...
def daily_return_cal(prc):
    ...
    for i in range(1, len(prc)):
        daily_return = (prc[i] - prc[i - 1]) / prc[i - 1]
        all_daily_return.append(daily_return)

    daily_return_2 = pd.Series(all_daily_return, index=prc.index[1:], name=prc.name)
    return daily_return_2.dropna()
...
def aj_ret_dict(tickers, start, end):
    ...
    for tic in tickers:
        ...
        now_daily_return = daily_return_cal(prc)
        all_daily_return.append(now_daily_return.rename(tic.lower()))

        now_monthly_return = monthly_return_cal(prc)
        all_monthly_return.append(now_monthly_return.rename(tic.lower()))

    daily_return_df = pd.concat(all_daily_return, axis=1)
    monthly_return_df = pd.concat(all_monthly_return, axis=1)

    dict = {"Daily": daily_return_df, "Monthly": monthly_return_df}
