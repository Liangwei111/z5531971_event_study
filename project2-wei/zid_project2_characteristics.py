...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    # <COMPLETE THIS PART>
    # Initialize an empty dictionary to store the volatility data frames
    vol_dict = {}

    # Loop through each frequency specified in ret_freq_use
    for freq in ret_freq_use:
        # Make sure the frequency exists in the ret dictionary
        if freq in ret:
            # Copy the data frame to avoid modifying the original data
            df_freq = ret[freq].copy()

            # Calculate the standard deviation (volatility) for each month
            vol = df_freq.resample('ME').std()

            # Count the number of entries per month to filter out months with fewer than 18 data points
            count = df_freq.resample('ME').count()

            # Set volatility to NaN for months with fewer than 18 entries
            vol[count < 18] = None

            # Store the resulting DataFrame in the vol_dict dictionary
            vol_dict[f"{freq}_{cha_name}"] = vol

    # If there's data to merge (i.e., if vol_dict is not empty)
    if vol_dict:
        # Convert dictionary of dataframes into a single dataframe
        df_vol = pd.concat(vol_dict.values(), axis=1)
        # Drop all NaN rows as per the hint
        df_vol = df_vol.dropna(how='all')

        # Convert the index to a PeriodIndex for year-month format
        df_vol.index = df_vol.index.to_period('M')
        df_vol.index.name = 'Year_Month'

        # Add the suffix to the columns
        df_vol.columns = [f"{col}_{cha_name}" for col in df_vol.columns]

        return df_vol
    else:
        # Return an empty DataFrame if no valid data was found
        return pd.DataFrame()
...
def merge_tables(ret, df_cha, cha_name):
    ...
    """
    # <COMPLETE THIS PART>
    # Copy the monthly returns DataFrame to avoid modifying the original
    monthly_ret_df = ret['Monthly'].copy()

    # Shift the characteristics DataFrame one month forward
    df_cha_shifted = df_cha.shift(1)

    # Merge the monthly returns with the shifted characteristics
...
def cha_main(ret, cha_name, ret_freq_use: list):
    ...
    # Calculate the stock characteristics using the daily returns
    df_cha = vol_cal(ret, cha_name, ret_freq_use)

    # Merge the calculated characteristics with the monthly returns
